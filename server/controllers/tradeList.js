...
const getTradeLists = async (req, res) => {
  ...
  const tradeLists = await TradeList.find({ createdBy: userId }).sort(
    "createdAt"
  );
...
const getOneTradeList = async (req, res) => {
  const {
    user: userId,
    params: { id: tradeListId },
  } = req;
  const tradeList = await TradeList.findOne({
    _id: tradeListId,
...
const deleteTradeList = async (req, res) => {
  ...
  const tradeList = await TradeList.findOneAndDelete({
    _id: id,
    createdBy: userId,
  });
  if (!tradeList) {
