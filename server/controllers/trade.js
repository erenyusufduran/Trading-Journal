...
const getTrades = async (req, res) => {
  ...
  const trades = await Trade.find({ createdBy: userId, belongTo: id }).sort(
    "createdAt"
  );
...
const createTrade = async (req, res) => {
  ...
  if (!pair || !shortLong || !entry || !size) {
    throw new BadRequestError(
      "Please provide pair, short/long, entry and size."
    );
  }
...
const updateTrade = async (req, res) => {
  ...
  if (tp && sl) {
    ...
    if (wl) {
      ...
      trade.profit = trade.calculateProfitOrLoss(
        shortLong,
        size,
        entry,
        tp,
  ...
  const updatedTrade = await Trade.findOneAndUpdate({ id: tradeID }, trade, {
    new: true,
    runValidators: true,
  });
